...
// Read more on https://github.com/Cledersonbc/tic-tac-toe-Maxmin/
function Maxmin(state, depth, player) {
	var best;
	...
	emptyCells(state).forEach(function (cell) {
		...
		var y = cell[1];
		state[x][y] = player;
		var score = Maxmin(state, depth - 1, -player);
		state[x][y] = 0;
		score[0] = x;
		score[1] = y;
...
function aiTurn() {
	...
	if (emptyCells(board).length == 9) {
		...
		move = Maxmin(board, emptyCells(board).length, COMP);
		x = move[0];
		y = move[1];
	}
